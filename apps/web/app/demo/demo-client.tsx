...
export function DemoClient() {
  ...
  async function startDemo() { setLoading(true); setError(undefined); try { await apiFetch("/api/demo/session", { method: "POST" }); window.location.assign("/analyzer"); } catch (cause) { setError(cause instanceof Error ? cause.message : "Unable to begin demo."); } finally { setLoading(false); } }
