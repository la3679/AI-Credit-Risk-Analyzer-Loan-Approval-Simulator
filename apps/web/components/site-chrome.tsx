...
export function SiteChrome({ children }: { children: React.ReactNode }) {
  ...
  return <><header className="sticky top-0 z-20 border-b border-[#b9eae11c] bg-[#071411dd] backdrop-blur"><nav className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4"><Link href="/" className="font-semibold tracking-tight">{APP_BRAND.shortName}<span className="text-[#61e6cf]">.ai</span></Link><div className="hidden gap-5 text-sm text-[#b8d3cf] md:flex"><Link href="/product">Product</Link><Link href="/how-it-works">How it works</Link><Link href="/pricing">Pricing</Link><Link href="/loan-simulator">Simulator</Link><Link href="/dashboard">Workspace</Link></div><div className="flex items-center gap-2">{user ? <><span className="hidden text-xs text-[#9cb7b5] sm:inline">{user.name}</span><button className="button button-secondary text-sm" onClick={() => void signOut()}>Sign out</button></> : !loading && <><Link className="button button-secondary text-sm" href="/login">Sign in</Link><Link className="button button-primary text-sm" href="/demo">Try demo</Link></>}</div></nav></header><main>{children}</main><MobileNav/><footer className="mx-auto max-w-7xl px-5 py-10"><div className="border-t border-[#b9eae11c] pt-6"><Disclaimer compact /><p className="mt-3 text-xs text-[#6d8a86]">© 2026 {APP_BRAND.name}. Built as an engineering portfolio project.</p></div></footer></>;
}
