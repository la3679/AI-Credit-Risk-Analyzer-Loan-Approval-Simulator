...
export interface AIProvider {
  ...
  generateUnderwritingMemo(input: unknown): Promise<string>;
}

const systemPrompt = `Write a concise educational credit-simulation memo. Never state or imply a real lending decision, regulatory compliance, financial advice, or real-credit-report access. Include this disclaimer verbatim: ${SIMULATOR_DISCLAIMER}`;
...
class OpenAICompatibleProvider implements AIProvider {
  ...
  async generateUnderwritingMemo(input: unknown) {
    const response = await fetch(`${this.baseUrl.replace(/\/$/, "")}/chat/completions`, { method: "POST", headers: { "content-type": "application/json", authorization: `Bearer ${this.apiKey}` }, body: JSON.stringify({ model: this.model, temperature: 0.2, messages: [{ role: "system", content: systemPrompt }, { role: "user", content: JSON.stringify(input) }] }) });
...
class GeminiProvider implements AIProvider {
  ...
  async generateUnderwritingMemo(input: unknown) {
    ...
    const response = await fetch(endpoint, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ contents: [{ parts: [{ text: `${systemPrompt}\n\n${JSON.stringify(input)}` }] }] }) });
